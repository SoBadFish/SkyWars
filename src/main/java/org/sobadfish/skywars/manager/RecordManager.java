...
public class RecordManager {
            ...
            public void onRun(int i) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (playbackEngine.isStopped()) {
                        ...
                        TotalManager.sendMessageToObject(levelName + " 录像播放完成，已删除回放地图", player);
                        iterator.remove();
                    }
                }
            }
        };
        Server.getInstance().getScheduler().scheduleRepeatingTask(TotalManager.getPlugin(), this.recordTask, 20);
    }

    private String getRecordPath() {
        return TotalManager.getDataFolder() + "/record/";
    }
    ...
    public void stopRecord(@NotNull GameRoom room) {
        ...
        try {
            ...
            //TODO 添加其他信息
            File file = new File(this.getRecordPath() + room.getRoomConfig().getName() + "-" + time + ".swRecord");
            recordPack.packFile(file);
        } catch (Throwable e) {
            TotalManager.getPlugin().getLogger().error("游戏录制保存失败", e);
        }
    }
    ...
    public OK playRecord(File file, Player player) {
        try {
            ...
            TotalManager.getPlugin().getLogger().error("游戏录像读取失败", e);
    ...
    public List<File> getRecordFileList() {
        ...
        if (files != null) {
            ...
            for (File file : files) {
                if (!file.isDirectory() && file.getName().endsWith(".swRecord")) {
                    list.add(file);
                }
            }
            return list;
